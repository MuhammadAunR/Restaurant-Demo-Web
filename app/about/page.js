...
const AboutUs = () => {
  return (
    <section className='bg-mist-900 relative overflow-hidden text-white'>

      {/* ── Decorative background orbs ── */}
      <div className='absolute top-0 left-0 w-64 h-64 md:w-96 md:h-96 rounded-full bg-(--color-primary)/5 blur-3xl pointer-events-none' />
      <div className='absolute bottom-0 right-0 w-56 h-56 md:w-80 md:h-80 rounded-full bg-(--color-primary)/5 blur-3xl pointer-events-none' />

      {/* ── Section Header ── */}
      <div className='relative z-10 flex flex-col items-center gap-3 pt-16 md:pt-24 pb-10 md:pb-16 px-6 text-center'>
        <span className='font-stylish text-xl md:text-2xl text-(--color-primary) tracking-wide'>
          About Us
        </span>

        <div className='flex items-center gap-3'>
          <span className='h-px w-12 bg-(--color-primary)/50'></span>
          <span className='w-1.5 h-1.5 rotate-45 bg-(--color-primary)'></span>
          <span className='h-px w-12 bg-(--color-primary)/50'></span>
        </div>

        <h1 className='font-heading text-3xl sm:text-4xl md:text-5xl font-bold uppercase tracking-widest text-(--color-primary-light) mt-2'>
          Our Story
        </h1>
      ...
      <div className='relative z-10 max-w-7xl mx-auto px-6 sm:px-10 pb-16 md:pb-24'>
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center'>
          ...
          {/* ── Left — Images ── */}
          <div className='relative h-72 sm:h-96 lg:h-[580px]'>

            {/* Large image */}
            <div className='absolute top-0 left-0 w-44 sm:w-60 lg:w-72 h-60 sm:h-80 lg:h-96 overflow-hidden'>
              <div className='absolute inset-0 ring-1 ring-(--color-primary)/30 z-10 pointer-events-none' />
              <img
            ...
            {/* Small image bottom-right — hidden on mobile */}
            ...
            {/* Gold accent badge */}
            <div className='absolute bottom-0 left-0 bg-mist-800 border border-(--color-primary)/30 px-4 sm:px-6 py-3 sm:py-4'>
              <p className='font-stylish text-(--color-primary) text-base sm:text-lg'>Since 2012</p>
          ...
          <div className='flex flex-col gap-6 lg:gap-8'>
            ...
            <div className='flex flex-col gap-4 lg:gap-5'>
              <p className='text-white/75 leading-7 lg:leading-8 text-sm sm:text-base'>
                At <span className='text-(--color-primary) font-semibold'>DMR Sushi</span>, our story began
            ...
            <div className='flex items-center gap-4'>
              <span className='h-px flex-1 bg-(--color-primary)/20'></span>
              <span className='w-1.5 h-1.5 rotate-45 bg-(--color-primary)/50'></span>
              <span className='h-px flex-1 bg-(--color-primary)/20'></span>
            </div>
            ...
            <div className='grid grid-cols-2 sm:grid-cols-4 gap-3 lg:gap-4'>
              {stats.map((stat) => (
                <div key={stat.label} className='flex flex-col items-center gap-1 border border-(--color-primary)/20 py-3 lg:py-4 px-2 hover:border-(--color-primary)/60 transition-colors duration-300'>
                  <span className='font-heading text-xl lg:text-2xl font-bold text-(--color-primary)'>{stat.value}</span>
      ...
      {/* ── Chef Quote Banner ── */}
      <div className='relative z-10 border-t border-b border-(--color-primary)/20 py-8 lg:py-10 px-6 lg:px-10 bg-(--color-primary)/5'>
        <div className='max-w-3xl mx-auto text-center flex flex-col items-center gap-4'>
          <span className='text-(--color-primary) text-4xl lg:text-5xl font-stylish leading-none'>"</span>
          ...
          <div className='flex items-center gap-3 mt-2'>
            <span className='h-px w-8 bg-(--color-primary)/50'></span>
            <span className='text-(--color-primary) text-xs sm:text-sm uppercase tracking-widest'>Chef Damir</span>
            <span className='h-px w-8 bg-(--color-primary)/50'></span>
          </div>
        </div>
      </div>
